/*
 * RTL-Injector for Wi-Fi packet injection on AmebaDplus (RTL8721Dx)
 *
 * This header provides the interface to create, configure and control
 * multiple independent frame injectors.
 */
...
/* Ameba Wi‑Fi headers – correct for current SDK */
#include "wifi_api.h"
#include "wifi_api_types.h"
#include "wifi_api_ext.h"

/* -------------------------------------------------------------------------
 * Weak default implementations for platform time (can be overridden)
 * ------------------------------------------------------------------------- */
__attribute__((weak)) uint64_t platform_get_time_ns(void) {
    /* On Cortex‑M, use DWT cycle counter */
    extern uint32_t SystemCoreClock;
    static uint32_t cyccnt_per_ns = 0;
    if (cyccnt_per_ns == 0) {
        cyccnt_per_ns = SystemCoreClock / 1000000;  /* cycles per µs */
...
/* Map inject_rate_t to the numeric values used by the driver.
 * The driver expects one of the RTW_RATE_* macros (or MGN_RATE values).
 * If RTW_RATE_* are not defined, we fall back to MGN_RATE (see AN1003). */
#ifndef RTW_RATE_1M
/* Fallback to MGN_RATE values (from AN1003) */
...
/* Send a raw frame using the driver's correct structure */
static int send_packet(PacketInjector *inj) {
    struct rtw_raw_frame_desc desc;          /* correct type name */
    memset(&desc, 0, sizeof(desc));
    desc.wlan_idx = STA_WLAN_INDEX;          /* defined in wifi_conf.h */
    ...
    desc.ac_queue = 0;                       /* BE queue */
    desc.sgi = (inj->flags & INJ_FLAG_USE_SHORT_GI) ? 1 : 0;
    desc.agg_en = (inj->flags & INJ_FLAG_AGGREGATE) ? 1 : 0;
    /* Note: power control not in rtw_raw_frame_desc – implement if driver supports it */
    return wifi_send_raw_frame(&desc);
}

/* -------------------------------------------------------------------------
 * Public API (unchanged, except where noted)
 * ------------------------------------------------------------------------- */
...
int injectorManager_startInjectorEx(injectorManager *mgr,
    ...
    inj->packetData = copy;
#else
    inj->packetData = (uint8_t*)packetData;   /* cast away const – user must keep buffer valid */
#endif
...
int injectorManager_activateInjector(injectorManager *mgr, const char *injectorName) {
    ...
    inj->active = true;
    /* Reset next_time to now so it sends immediately */
...
/* Generic helper for setting a field */
...
int injectorManager_setInjectorPacketData(injectorManager *mgr, const char *injectorName,
    ...
    inj->packetData = copy;
#else
    /* Without copying, we just replace the pointer – user must ensure old buffer remains valid? */
...
int injectorManager_schedule(injectorManager *mgr, uint64_t now_ns) {
    ...
    for (int i = 0; i < INJECTOR_MAX; i++) {
        ...
        /* Time to send – switch channel if needed */
        if (!(inj->flags & INJ_FLAG_FIXED_CHANNEL) && mgr->current_channel != inj->channel) {
            if (wifi_set_channel(STA_WLAN_INDEX, inj->channel) == 0) {
                ...
                /* Channel switch failed; try again later */
                if (inj->next_time_ns < next_wake) next_wake = inj->next_time_ns;
                continue;
            }
        }
        ...
        if (ret == 0) {
            ...
            while (inj->next_time_ns <= now_ns) {
                inj->next_time_ns += inj->interval_ns;   /* catch up */
            }
            if (inj->next_time_ns < next_wake) next_wake = inj->next_time_ns;
            ...
            if (inj->maxRetries > 0 && inj->retry_count > inj->maxRetries) {
                ...
                /* Keep same next_time to retry later */
                if (inj->next_time_ns < next_wake) next_wake = inj->next_time_ns;
            }
        } else {
            /* Other error, deactivate */
            inj->active = false;
#if INJECT_COPY_PACKET_DATA
            if (inj->packetData) {
                vPortFree(inj->packetData);
...
static void scheduler_task(void *param) {
    ...
    while (1) {
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);   /* Wait for timer IRQ */
...
/* Timer ISR (example) – must be hooked to hardware timer compare */
void injector_timer_isr(void) {
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;
    /* Assume mgr is a global or passed via a pointer */
