...
#define INJECT_COPY_PACKET_DATA 1   /* 1 = copy packet data to internal heap (recommended) */
#endif

/* TX rates (matching Ameba rate definitions) */
typedef enum {
    INJ_RATE_1M,
    INJ_RATE_2M,
    INJ_RATE_5_5M,
...
typedef enum {
    INJ_FLAG_NONE           = 0,
    INJ_FLAG_NO_ACK         = (1 << 0),   /* disable ACK expectation */
    INJ_FLAG_USE_SHORT_GI   = (1 << 1),   /* short guard interval */
    INJ_FLAG_AGGREGATE      = (1 << 2),   /* allow aggregation */
    INJ_FLAG_FIXED_CHANNEL  = (1 << 3),   /* do not auto-switch channel */
} inject_flags_t;

/* Structure defining an injector (internal state) */
typedef struct {
    char name[INJECTOR_NAME_MAX];
    bool active;
    uint8_t channel;                /* Wi‑Fi channel (1-14, or 5 GHz channels) */
    uint32_t interval_ns;           /* interval between packets in nanoseconds */
    uint64_t next_time_ns;          /* absolute next send time (nanoseconds since epoch) */
    uint64_t start_time_ns;         /* scheduled start time (0 = immediate) */
    uint32_t maxPackets;            /* 0 = unlimited */
    uint32_t packets_sent;          /* counter */
    uint8_t  maxRetries;            /* max retries before deactivating */
    uint8_t  retry_count;           /* current consecutive failures */
    inject_rate_t tx_rate;          /* transmission rate */
    int8_t   tx_power_dbm;          /* transmit power in dBm (if supported, else -1) */
    inject_flags_t flags;           /* additional flags */
    uint8_t *packetData;            /* pointer to packet buffer (owned if copied) */
    uint32_t packetLen;
} PacketInjector;

/* Structure for retrieving injector information (public snapshot) */
typedef struct {
    char name[INJECTOR_NAME_MAX];
    bool active;
    uint8_t channel;
    ...
    uint32_t packetLen;
    /* (packet data not included for size reasons) */
} InjectorInfo;

/* Manager structure */
typedef struct {
    PacketInjector injectors[INJECTOR_MAX];
    uint8_t injectorCount;           /* number of active injectors */
    uint64_t totalPacketsThisRun;
    uint64_t totalPacketsAllTime;
    int current_channel;             /* current Wi‑Fi channel (-1 = unknown) */
#ifdef INJECT_USE_RTOS
    SemaphoreHandle_t lock;
    TaskHandle_t scheduler_task;     /* handle of the scheduler task */
#endif
} injectorManager;

/* -------------------------------------------------------------------------
 * Initialisation and global control
 * ------------------------------------------------------------------------- */
void injectorManager_init(injectorManager *mgr);
void injectorManager_clearAllInjectors(injectorManager *mgr);

/* -------------------------------------------------------------------------
 * Starting injectors (with full parameters)
 * ------------------------------------------------------------------------- */
int injectorManager_startInjectorEx(injectorManager *mgr,
                                    ...
                                    uint32_t packetLen,
                                    uint8_t channel,
                                    uint64_t start_time_ns,      /* 0 = now */
                                    uint32_t interval_ns,        /* nanosecond interval */
                                    uint32_t maxPackets,
                                    uint8_t maxRetries,
                                    inject_rate_t rate,
                                    int8_t tx_power_dbm,
...
/* Simplified start (microsecond interval, backward compatible) */
...
/* -------------------------------------------------------------------------
 * Stopping and activating/deactivating injectors
 * ------------------------------------------------------------------------- */
...
/* Replace the packet data of a running injector (old data freed if copied) */
...
/* -------------------------------------------------------------------------
 * Scheduler (to be called periodically or from timer IRQ)
 * ------------------------------------------------------------------------- */
int injectorManager_schedule(injectorManager *mgr, uint64_t now_ns);

/* -------------------------------------------------------------------------
 * RTOS background task (optional) – guarded by INJECT_USE_RTOS
 * ------------------------------------------------------------------------- */
#ifdef INJECT_USE_RTOS
...
/* -------------------------------------------------------------------------
 * Platform‑specific time/timer functions (must be provided)
 * ------------------------------------------------------------------------- */
